...
export default function JurusanSection({ search, isKepsek }: JurusanSectionProps) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await api.delete(`/master/jurusan/${id}`);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editMode && selectedItem) {
        await api.put(`/master/jurusan/${selectedItem.id || selectedItem.ID}`, formData);
        toast.success("Data berhasil diperbarui");
      } else {
        await api.post("/master/jurusan", formData);
  ...
  const filteredData = data.filter((item) => {
    ...
    return (
      item.kode_jurusan?.toLowerCase().includes(q) ||
    ...
    <div className="space-y-4">
        ...
        <Table>
          <TableHeader className="border-border/30">
            <TableRow className="hover:bg-white/5 border-border/30">
              ...
              <TableHead className="text-gray-400">Kode Jurusan</TableHead>
              <TableHead className="text-gray-400">Nama Jurusan</TableHead>
          ...
          <TableBody>
            {filteredData.map((item) => (
              <TableRow key={item.id || item.ID} className="hover:bg-white/5 border-border/20">
                ...
                <TableCell className="text-white font-semibold">{item.kode_jurusan}</TableCell>
                <TableCell className="text-gray-300">{item.nama_jurusan}</TableCell>
                {!isKepsek && (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-[#111420] border-border/30 text-white max-w-md">
          <DialogHeader>
            <DialogTitle className="text-white">{editMode ? "Ubah Jurusan" : "Tambah Jurusan"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSave} className="space-y-4 py-2">
            <div className="space-y-1">
              <Label htmlFor="kode_jurusan" className="text-gray-300">Kode Jurusan</Label>
              <Input
                id="kode_jurusan"
            ...
            <div className="space-y-1">
              <Label htmlFor="nama_jurusan" className="text-gray-300">Nama Jurusan</Label>
              <Input
                id="nama_jurusan"
